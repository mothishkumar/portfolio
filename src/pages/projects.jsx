...
export const Projects = () => {
  ...
  return (
    <div className="p-8 rounded-lg shadow-xl">
      <h2 className="text-4xl mb-8 text-center">Personal Projects</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {projects.map((project, index) => (
          <motion.div
            key={index}
            className="bg-gray-100 p-6 rounded-xl shadow-lg transform transition-all hover:scale-105 hover:shadow-2xl hover:bg-gray-100"
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, type: 'spring', stiffness: 80 }}
          >
            <div className="text-2xl font-semibold text-gray-800 mb-4">{project.title}</div>
            <p className="text-gray-600 text-lg mb-4">{project.description}</p>
            <div className="flex flex-wrap gap-4 mb-6">
              {project.technologies.map((tech, index) => (
                <div key={index} className="flex items-center gap-2">
                  <span className="text-xl">{tech.icon}</span>
                  <span className="text-gray-600 text-md">{tech.name}</span>
                </div>
              ))}
            </div>
            <a
              ...
              className="inline-block text-white bg-purple-700 px-4 py-2 rounded-lg transition-all duration-200 hover:bg-purple-800"
            >
              View Project
            </a>
          </motion.div>
